import type { InsuranceSnapshot, InsuranceAuditTrail } from '../domain/types';
import type { InsuranceListItem, Page } from '../application/queries';
export interface InsuranceReadModel {
  byId(id: string): Promise<InsuranceSnapshot | null>;
  byCustomer(customerId: string, status?: string): Promise<InsuranceListItem[]>;
  search(q: string, riskTier: string | undefined, page: number, pageSize: number): Promise<Page<InsuranceListItem>>;
  audit(productId: string, limit: number): Promise<InsuranceAuditTrail[]>;
...
export class ElasticInsuranceReadModel implements InsuranceReadModel {
  constructor(
    private readonly es: {
      search: (body: unknown) => Promise<{ hits: { hits: { _source: InsuranceSnapshot }[]; total: { value: number } } }>;
      get: (id: string) => Promise<{ _source: InsuranceSnapshot } | null>;
    },
    private readonly index = 'insurance-read',
  ) {}
  async byId(id: string): Promise<InsuranceSnapshot | null> {
    const doc = await this.es.get(id);
    return doc?._source ?? null;
  }
  async byCustomer(customerId: string, status?: string): Promise<InsuranceListItem[]> {
  ...
  async search(q: string, riskTier: string | undefined, page: number, pageSize: number): Promise<Page<InsuranceListItem>> {
  ...
  async audit(_productId: string, _limit: number): Promise<InsuranceAuditTrail[]> {
    return [];
  }
  ...
  private toListItem(s: InsuranceSnapshot): InsuranceListItem {
    return {
      id: s.id,
...
export class InMemoryInsuranceReadModel implements InsuranceReadModel {
  private docs = new Map<string, InsuranceSnapshot>();
  private audits = new Map<string, InsuranceAuditTrail[]>();
  upsert(snap: InsuranceSnapshot, audit: InsuranceAuditTrail[] = []): void {
