...
import type { InsuranceSnapshot, InsuranceFeeSchedule, InsuranceLimits } from '../domain/types';
import { Money, type CurrencyCode } from '../../../../packages/domain-core/src';
export interface InsuranceRepository {
  findById(id: string): Promise<InsuranceAggregate | null>;
  save(agg: InsuranceAggregate): Promise<void>;
  listByCustomer(customerId: string): Promise<InsuranceSnapshot[]>;
}
/** In-memory repository used for local/dev and tests */
export class InMemoryInsuranceRepository implements InsuranceRepository {
  private store = new Map<string, InsuranceSnapshot>();
  private aggregates = new Map<string, InsuranceAggregate>();
  async findById(id: string): Promise<InsuranceAggregate | null> {
    return this.aggregates.get(id) ?? null;
  }
  async save(agg: InsuranceAggregate): Promise<void> {
    const snap = agg.toSnapshot();
    this.store.set(agg.id, snap);
    this.aggregates.set(agg.id, agg);
  }
  async listByCustomer(customerId: string): Promise<InsuranceSnapshot[]> {
    return [...this.store.values()].filter((s) => s.customerId === customerId);
  }
  /** Rehydrate helper for projections */
  static rehydrate(snap: InsuranceSnapshot): InsuranceAggregate {
    const fees: InsuranceFeeSchedule = snap.fees;
    const limits: InsuranceLimits = snap.limits;
...
export class PostgresInsuranceRepository implements InsuranceRepository {
  constructor(private readonly pool: { query: (sql: string, params?: unknown[]) => Promise<{ rows: InsuranceSnapshot[] }> }) {}
  async findById(id: string): Promise<InsuranceAggregate | null> {
    ...
    return InMemoryInsuranceRepository.rehydrate(rows[0]);
  }
  async save(agg: InsuranceAggregate): Promise<void> {
    const s = agg.toSnapshot();
    await this.pool.query(
      `INSERT INTO insurance (id, customer_id, status, risk_tier, balance_minor, currency, opened_at, updated_at, version, fees, limits, tags, metadata)
  ...
  async listByCustomer(customerId: string): Promise<InsuranceSnapshot[]> {
    const { rows } = await this.pool.query(
      'SELECT * FROM insurance WHERE customer_id = $1 ORDER BY opened_at DESC',
      [customerId],
    );
    return rows;
  }
