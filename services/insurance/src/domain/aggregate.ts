...
import type { InsuranceStatus, InsuranceRiskTier, InsuranceFeeSchedule, InsuranceLimits, InsuranceSnapshot, InsuranceAuditTrail } from './types';
export class InsuranceAggregate extends AggregateRoot {
  private status: InsuranceStatus = 'draft';
  private riskTier: InsuranceRiskTier = 'medium';
  private balance: Money;
  private version = 0;
  private audit: InsuranceAuditTrail[] = [];
  private tags: string[] = [];
  ...
  private constructor(
    ...
    private fees: InsuranceFeeSchedule,
    private limits: InsuranceLimits,
  ) {
    super(id);
  ...
  static open(
    ...
    const agg = new InsuranceAggregate(id, customerId, currency, fees, limits);
    agg.status = 'pending';
    agg.openedAt = new Date();
    agg.record('OPENED', actorId, { currency });
    agg.addDomainEvent(createEvent(id, 'insurance.opened', { customerId, currency }));
    return agg;
  }
  activate(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'insurance.activated', { customerId: this.customerId }));
  }
  suspend(actorId: string, reason: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'insurance.suspended', { reason }));
  }
  close(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'insurance.closed', {}));
  }
  credit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'insurance.credited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  debit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'insurance.debited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  ...
  updateLimits(limits: Partial<InsuranceLimits>, actorId: string): void {
  ...
  getAuditTrail(): InsuranceAuditTrail[] { return [...this.audit]; }
