import type { MortgagesSnapshot, MortgagesRiskTier } from './types';
export interface PolicyDecision {
  allowed: boolean;
  reasons: string[];
  requiredApprovals: number;
  riskImpact: MortgagesRiskTier;
}
export class MortgagesPolicyEngine {
  ...
  evaluateLargeDebit(snap: MortgagesSnapshot, amountMinor: number): PolicyDecision {
    const reasons: string[] = [];
  ...
  evaluateClosure(snap: MortgagesSnapshot): PolicyDecision {
    const reasons: string[] = [];
  ...
  scoreInterestEligibility(snap: MortgagesSnapshot, daysActive: number): number {
    let score = 50;
  ...
  computeProvisioning(snap: MortgagesSnapshot): number {
    const rates: Record<MortgagesRiskTier, number> = { low: 0.01, medium: 0.03, high: 0.1, critical: 0.25 };
