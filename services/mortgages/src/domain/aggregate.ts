...
import type { MortgagesStatus, MortgagesRiskTier, MortgagesFeeSchedule, MortgagesLimits, MortgagesSnapshot, MortgagesAuditTrail } from './types';
export class MortgagesAggregate extends AggregateRoot {
  private status: MortgagesStatus = 'draft';
  private riskTier: MortgagesRiskTier = 'medium';
  private balance: Money;
  private version = 0;
  private audit: MortgagesAuditTrail[] = [];
  private tags: string[] = [];
  ...
  private constructor(
    ...
    private fees: MortgagesFeeSchedule,
    private limits: MortgagesLimits,
  ) {
    super(id);
  ...
  static open(
    ...
    const agg = new MortgagesAggregate(id, customerId, currency, fees, limits);
    agg.status = 'pending';
    agg.openedAt = new Date();
    agg.record('OPENED', actorId, { currency });
    agg.addDomainEvent(createEvent(id, 'mortgages.opened', { customerId, currency }));
    return agg;
  }
  activate(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'mortgages.activated', { customerId: this.customerId }));
  }
  suspend(actorId: string, reason: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'mortgages.suspended', { reason }));
  }
  close(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'mortgages.closed', {}));
  }
  credit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'mortgages.credited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  debit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'mortgages.debited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  ...
  updateLimits(limits: Partial<MortgagesLimits>, actorId: string): void {
  ...
  getAuditTrail(): MortgagesAuditTrail[] { return [...this.audit]; }
