import type { CreditCardsSnapshot, CreditCardsAuditTrail } from '../domain/types';
import type { CreditCardsListItem, Page } from '../application/queries';
export interface CreditCardsReadModel {
  byId(id: string): Promise<CreditCardsSnapshot | null>;
  byCustomer(customerId: string, status?: string): Promise<CreditCardsListItem[]>;
  search(q: string, riskTier: string | undefined, page: number, pageSize: number): Promise<Page<CreditCardsListItem>>;
  audit(productId: string, limit: number): Promise<CreditCardsAuditTrail[]>;
...
export class ElasticCreditCardsReadModel implements CreditCardsReadModel {
  constructor(
    private readonly es: {
      search: (body: unknown) => Promise<{ hits: { hits: { _source: CreditCardsSnapshot }[]; total: { value: number } } }>;
      get: (id: string) => Promise<{ _source: CreditCardsSnapshot } | null>;
    },
    private readonly index = 'credit-cards-read',
  ) {}
  async byId(id: string): Promise<CreditCardsSnapshot | null> {
    const doc = await this.es.get(id);
    return doc?._source ?? null;
  }
  async byCustomer(customerId: string, status?: string): Promise<CreditCardsListItem[]> {
  ...
  async search(q: string, riskTier: string | undefined, page: number, pageSize: number): Promise<Page<CreditCardsListItem>> {
  ...
  async audit(_productId: string, _limit: number): Promise<CreditCardsAuditTrail[]> {
    return [];
  }
  ...
  private toListItem(s: CreditCardsSnapshot): CreditCardsListItem {
    return {
      id: s.id,
...
export class InMemoryCreditCardsReadModel implements CreditCardsReadModel {
  private docs = new Map<string, CreditCardsSnapshot>();
  private audits = new Map<string, CreditCardsAuditTrail[]>();
  upsert(snap: CreditCardsSnapshot, audit: CreditCardsAuditTrail[] = []): void {
