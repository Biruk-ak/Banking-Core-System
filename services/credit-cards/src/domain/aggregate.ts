...
import type { CreditCardsStatus, CreditCardsRiskTier, CreditCardsFeeSchedule, CreditCardsLimits, CreditCardsSnapshot, CreditCardsAuditTrail } from './types';
export class CreditCardsAggregate extends AggregateRoot {
  private status: CreditCardsStatus = 'draft';
  private riskTier: CreditCardsRiskTier = 'medium';
  private balance: Money;
  private version = 0;
  private audit: CreditCardsAuditTrail[] = [];
  private tags: string[] = [];
  ...
  private constructor(
    ...
    private fees: CreditCardsFeeSchedule,
    private limits: CreditCardsLimits,
  ) {
    super(id);
  ...
  static open(
    ...
    actorId: string,
  ): CreditCardsAggregate {
    const agg = new CreditCardsAggregate(id, customerId, currency, fees, limits);
    agg.status = 'pending';
    agg.openedAt = new Date();
    agg.record('OPENED', actorId, { currency });
    agg.addDomainEvent(createEvent(id, 'credit-cards.opened', { customerId, currency }));
    return agg;
  }
  activate(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'credit-cards.activated', { customerId: this.customerId }));
  }
  suspend(actorId: string, reason: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'credit-cards.suspended', { reason }));
  }
  close(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'credit-cards.closed', {}));
  }
  credit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'credit-cards.credited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  debit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'credit-cards.debited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  ...
  updateLimits(limits: Partial<CreditCardsLimits>, actorId: string): void {
  ...
  toSnapshot(): CreditCardsSnapshot {
    return {
      id: this.id,
  ...
  getAuditTrail(): CreditCardsAuditTrail[] { return [...this.audit]; }
