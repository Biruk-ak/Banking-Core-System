import type { CreditCardsSnapshot, CreditCardsRiskTier } from './types';
export interface PolicyDecision {
  allowed: boolean;
  reasons: string[];
  requiredApprovals: number;
  riskImpact: CreditCardsRiskTier;
}
export class CreditCardsPolicyEngine {
  ...
  evaluateLargeDebit(snap: CreditCardsSnapshot, amountMinor: number): PolicyDecision {
    const reasons: string[] = [];
  ...
  evaluateClosure(snap: CreditCardsSnapshot): PolicyDecision {
    const reasons: string[] = [];
  ...
  scoreInterestEligibility(snap: CreditCardsSnapshot, daysActive: number): number {
    let score = 50;
  ...
  computeProvisioning(snap: CreditCardsSnapshot): number {
    const rates: Record<CreditCardsRiskTier, number> = { low: 0.01, medium: 0.03, high: 0.1, critical: 0.25 };
