...
import type { ForeignExchangeCommand, CommandResult } from './commands';
import type { ForeignExchangeFeeSchedule, ForeignExchangeLimits } from '../domain/types';
import type { EventBus } from '../../../shared/infra/event-bus';
import type { ForeignExchangeRepository } from '../infrastructure/repository';
const DEFAULT_FEES: ForeignExchangeFeeSchedule = {
...
const DEFAULT_LIMITS: ForeignExchangeLimits = {
...
export class ForeignExchangeCommandHandler {
  constructor(
    private readonly repo: ForeignExchangeRepository,
    private readonly bus: EventBus,
  ) {}
  async handle(cmd: ForeignExchangeCommand): Promise<CommandResult> {
    try {
      switch (cmd.type) {
        case 'OpenForeignExchange': {
        ...
        case 'ReassessForeignExchangeRisk': {
          const agg = await this.load(cmd.productId);
          agg.reassessRisk(cmd.score, cmd.actorId);
          await this.persist(agg);
          return { success: true, productId: cmd.productId };
        }
        case 'UpdateForeignExchangeLimits': {
          const agg = await this.load(cmd.productId);
  ...
  private async load(id: string): Promise<ForeignExchangeAggregate> {
    const found = await this.repo.findById(id);
    if (!found) throw new Error('ForeignExchange not found: ' + id);
    return found;
  }
  private async persist(agg: ForeignExchangeAggregate): Promise<void> {
