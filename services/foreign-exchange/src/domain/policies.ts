import type { ForeignExchangeSnapshot, ForeignExchangeRiskTier } from './types';
export interface PolicyDecision {
  allowed: boolean;
  reasons: string[];
  requiredApprovals: number;
  riskImpact: ForeignExchangeRiskTier;
}
export class ForeignExchangePolicyEngine {
  ...
  evaluateLargeDebit(snap: ForeignExchangeSnapshot, amountMinor: number): PolicyDecision {
    const reasons: string[] = [];
  ...
  evaluateClosure(snap: ForeignExchangeSnapshot): PolicyDecision {
    const reasons: string[] = [];
  ...
  scoreInterestEligibility(snap: ForeignExchangeSnapshot, daysActive: number): number {
    let score = 50;
  ...
  computeProvisioning(snap: ForeignExchangeSnapshot): number {
    const rates: Record<ForeignExchangeRiskTier, number> = { low: 0.01, medium: 0.03, high: 0.1, critical: 0.25 };
