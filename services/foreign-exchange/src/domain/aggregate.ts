...
import type { ForeignExchangeStatus, ForeignExchangeRiskTier, ForeignExchangeFeeSchedule, ForeignExchangeLimits, ForeignExchangeSnapshot, ForeignExchangeAuditTrail } from './types';
export class ForeignExchangeAggregate extends AggregateRoot {
  private status: ForeignExchangeStatus = 'draft';
  private riskTier: ForeignExchangeRiskTier = 'medium';
  private balance: Money;
  private version = 0;
  private audit: ForeignExchangeAuditTrail[] = [];
  private tags: string[] = [];
  ...
  private constructor(
    ...
    private fees: ForeignExchangeFeeSchedule,
    private limits: ForeignExchangeLimits,
  ) {
    super(id);
  ...
  static open(
    ...
    actorId: string,
  ): ForeignExchangeAggregate {
    const agg = new ForeignExchangeAggregate(id, customerId, currency, fees, limits);
    agg.status = 'pending';
    agg.openedAt = new Date();
    agg.record('OPENED', actorId, { currency });
    agg.addDomainEvent(createEvent(id, 'foreign-exchange.opened', { customerId, currency }));
    return agg;
  }
  activate(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'foreign-exchange.activated', { customerId: this.customerId }));
  }
  suspend(actorId: string, reason: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'foreign-exchange.suspended', { reason }));
  }
  close(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'foreign-exchange.closed', {}));
  }
  credit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'foreign-exchange.credited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  debit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'foreign-exchange.debited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  ...
  updateLimits(limits: Partial<ForeignExchangeLimits>, actorId: string): void {
  ...
  toSnapshot(): ForeignExchangeSnapshot {
    return {
      id: this.id,
  ...
  getAuditTrail(): ForeignExchangeAuditTrail[] { return [...this.audit]; }
