...
import type { MobileBankingStatus, MobileBankingRiskTier, MobileBankingFeeSchedule, MobileBankingLimits, MobileBankingSnapshot, MobileBankingAuditTrail } from './types';
export class MobileBankingAggregate extends AggregateRoot {
  private status: MobileBankingStatus = 'draft';
  private riskTier: MobileBankingRiskTier = 'medium';
  private balance: Money;
  private version = 0;
  private audit: MobileBankingAuditTrail[] = [];
  private tags: string[] = [];
  ...
  private constructor(
    ...
    private fees: MobileBankingFeeSchedule,
    private limits: MobileBankingLimits,
  ) {
    super(id);
  ...
  static open(
    ...
    actorId: string,
  ): MobileBankingAggregate {
    const agg = new MobileBankingAggregate(id, customerId, currency, fees, limits);
    agg.status = 'pending';
    agg.openedAt = new Date();
    agg.record('OPENED', actorId, { currency });
    agg.addDomainEvent(createEvent(id, 'mobile-banking.opened', { customerId, currency }));
    return agg;
  }
  activate(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'mobile-banking.activated', { customerId: this.customerId }));
  }
  suspend(actorId: string, reason: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'mobile-banking.suspended', { reason }));
  }
  close(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'mobile-banking.closed', {}));
  }
  credit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'mobile-banking.credited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  debit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'mobile-banking.debited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  ...
  updateLimits(limits: Partial<MobileBankingLimits>, actorId: string): void {
  ...
  toSnapshot(): MobileBankingSnapshot {
    return {
      id: this.id,
  ...
  getAuditTrail(): MobileBankingAuditTrail[] { return [...this.audit]; }
