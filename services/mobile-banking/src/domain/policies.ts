import type { MobileBankingSnapshot, MobileBankingRiskTier } from './types';
export interface PolicyDecision {
  allowed: boolean;
  reasons: string[];
  requiredApprovals: number;
  riskImpact: MobileBankingRiskTier;
}
export class MobileBankingPolicyEngine {
  ...
  evaluateLargeDebit(snap: MobileBankingSnapshot, amountMinor: number): PolicyDecision {
    const reasons: string[] = [];
  ...
  evaluateClosure(snap: MobileBankingSnapshot): PolicyDecision {
    const reasons: string[] = [];
  ...
  scoreInterestEligibility(snap: MobileBankingSnapshot, daysActive: number): number {
    let score = 50;
  ...
  computeProvisioning(snap: MobileBankingSnapshot): number {
    const rates: Record<MobileBankingRiskTier, number> = { low: 0.01, medium: 0.03, high: 0.1, critical: 0.25 };
