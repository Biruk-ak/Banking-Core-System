import type { CurrentAccountsSnapshot, CurrentAccountsRiskTier } from './types';
export interface PolicyDecision {
  allowed: boolean;
  reasons: string[];
  requiredApprovals: number;
  riskImpact: CurrentAccountsRiskTier;
}
export class CurrentAccountsPolicyEngine {
  ...
  evaluateLargeDebit(snap: CurrentAccountsSnapshot, amountMinor: number): PolicyDecision {
    const reasons: string[] = [];
  ...
  evaluateClosure(snap: CurrentAccountsSnapshot): PolicyDecision {
    const reasons: string[] = [];
  ...
  scoreInterestEligibility(snap: CurrentAccountsSnapshot, daysActive: number): number {
    let score = 50;
  ...
  computeProvisioning(snap: CurrentAccountsSnapshot): number {
    const rates: Record<CurrentAccountsRiskTier, number> = { low: 0.01, medium: 0.03, high: 0.1, critical: 0.25 };
