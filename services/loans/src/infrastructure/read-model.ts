import type { LoansSnapshot, LoansAuditTrail } from '../domain/types';
import type { LoansListItem, Page } from '../application/queries';
export interface LoansReadModel {
  byId(id: string): Promise<LoansSnapshot | null>;
  byCustomer(customerId: string, status?: string): Promise<LoansListItem[]>;
  search(q: string, riskTier: string | undefined, page: number, pageSize: number): Promise<Page<LoansListItem>>;
  audit(productId: string, limit: number): Promise<LoansAuditTrail[]>;
...
export class ElasticLoansReadModel implements LoansReadModel {
  constructor(
    private readonly es: {
      search: (body: unknown) => Promise<{ hits: { hits: { _source: LoansSnapshot }[]; total: { value: number } } }>;
      get: (id: string) => Promise<{ _source: LoansSnapshot } | null>;
    },
    private readonly index = 'loans-read',
  ) {}
  async byId(id: string): Promise<LoansSnapshot | null> {
    const doc = await this.es.get(id);
    return doc?._source ?? null;
  }
  async byCustomer(customerId: string, status?: string): Promise<LoansListItem[]> {
  ...
  async search(q: string, riskTier: string | undefined, page: number, pageSize: number): Promise<Page<LoansListItem>> {
  ...
  async audit(_productId: string, _limit: number): Promise<LoansAuditTrail[]> {
    return [];
  }
  ...
  private toListItem(s: LoansSnapshot): LoansListItem {
    return {
      id: s.id,
...
export class InMemoryLoansReadModel implements LoansReadModel {
  private docs = new Map<string, LoansSnapshot>();
  private audits = new Map<string, LoansAuditTrail[]>();
  upsert(snap: LoansSnapshot, audit: LoansAuditTrail[] = []): void {
