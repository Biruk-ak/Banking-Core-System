import type { LoansSnapshot, LoansRiskTier } from './types';
export interface PolicyDecision {
  allowed: boolean;
  reasons: string[];
  requiredApprovals: number;
  riskImpact: LoansRiskTier;
}
export class LoansPolicyEngine {
  ...
  evaluateLargeDebit(snap: LoansSnapshot, amountMinor: number): PolicyDecision {
    const reasons: string[] = [];
  ...
  evaluateClosure(snap: LoansSnapshot): PolicyDecision {
    const reasons: string[] = [];
  ...
  scoreInterestEligibility(snap: LoansSnapshot, daysActive: number): number {
    let score = 50;
  ...
  computeProvisioning(snap: LoansSnapshot): number {
    const rates: Record<LoansRiskTier, number> = { low: 0.01, medium: 0.03, high: 0.1, critical: 0.25 };
