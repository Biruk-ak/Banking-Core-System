...
import type { LoansStatus, LoansRiskTier, LoansFeeSchedule, LoansLimits, LoansSnapshot, LoansAuditTrail } from './types';
export class LoansAggregate extends AggregateRoot {
  private status: LoansStatus = 'draft';
  private riskTier: LoansRiskTier = 'medium';
  private balance: Money;
  private version = 0;
  private audit: LoansAuditTrail[] = [];
  private tags: string[] = [];
  ...
  private constructor(
    ...
    private fees: LoansFeeSchedule,
    private limits: LoansLimits,
  ) {
    super(id);
  ...
  static open(
    ...
    const agg = new LoansAggregate(id, customerId, currency, fees, limits);
    agg.status = 'pending';
    agg.openedAt = new Date();
    agg.record('OPENED', actorId, { currency });
    agg.addDomainEvent(createEvent(id, 'loans.opened', { customerId, currency }));
    return agg;
  }
  activate(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'loans.activated', { customerId: this.customerId }));
  }
  suspend(actorId: string, reason: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'loans.suspended', { reason }));
  }
  close(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'loans.closed', {}));
  }
  credit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'loans.credited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  debit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'loans.debited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  ...
  updateLimits(limits: Partial<LoansLimits>, actorId: string): void {
  ...
  getAuditTrail(): LoansAuditTrail[] { return [...this.audit]; }
