import type { SavingsSnapshot, SavingsAuditTrail } from '../domain/types';
import type { SavingsListItem, Page } from '../application/queries';
export interface SavingsReadModel {
  byId(id: string): Promise<SavingsSnapshot | null>;
  byCustomer(customerId: string, status?: string): Promise<SavingsListItem[]>;
  search(q: string, riskTier: string | undefined, page: number, pageSize: number): Promise<Page<SavingsListItem>>;
  audit(productId: string, limit: number): Promise<SavingsAuditTrail[]>;
...
export class ElasticSavingsReadModel implements SavingsReadModel {
  constructor(
    private readonly es: {
      search: (body: unknown) => Promise<{ hits: { hits: { _source: SavingsSnapshot }[]; total: { value: number } } }>;
      get: (id: string) => Promise<{ _source: SavingsSnapshot } | null>;
    },
    private readonly index = 'savings-read',
  ) {}
  async byId(id: string): Promise<SavingsSnapshot | null> {
    const doc = await this.es.get(id);
    return doc?._source ?? null;
  }
  async byCustomer(customerId: string, status?: string): Promise<SavingsListItem[]> {
  ...
  async search(q: string, riskTier: string | undefined, page: number, pageSize: number): Promise<Page<SavingsListItem>> {
  ...
  async audit(_productId: string, _limit: number): Promise<SavingsAuditTrail[]> {
    return [];
  }
  ...
  private toListItem(s: SavingsSnapshot): SavingsListItem {
    return {
      id: s.id,
...
export class InMemorySavingsReadModel implements SavingsReadModel {
  private docs = new Map<string, SavingsSnapshot>();
  private audits = new Map<string, SavingsAuditTrail[]>();
  upsert(snap: SavingsSnapshot, audit: SavingsAuditTrail[] = []): void {
