import type { SavingsSnapshot, SavingsRiskTier } from './types';
export interface PolicyDecision {
  allowed: boolean;
  reasons: string[];
  requiredApprovals: number;
  riskImpact: SavingsRiskTier;
}
export class SavingsPolicyEngine {
  ...
  evaluateLargeDebit(snap: SavingsSnapshot, amountMinor: number): PolicyDecision {
    const reasons: string[] = [];
  ...
  evaluateClosure(snap: SavingsSnapshot): PolicyDecision {
    const reasons: string[] = [];
  ...
  scoreInterestEligibility(snap: SavingsSnapshot, daysActive: number): number {
    let score = 50;
  ...
  computeProvisioning(snap: SavingsSnapshot): number {
    const rates: Record<SavingsRiskTier, number> = { low: 0.01, medium: 0.03, high: 0.1, critical: 0.25 };
