/** Chargeback workflow — Banking Core System */
...
const TRANSITIONS: Record<ChargebackStage, ChargebackStage[]> = {
...
export class ChargebackWorkflow {
  private ctx: ChargebackContext;
  ...
  get context(): ChargebackContext { return { ...this.ctx, notes: [...this.ctx.notes], checklist: { ...this.ctx.checklist } }; }
  ...
  check(item: keyof ChargebackContext['checklist'], value = true): void {
  ...
  transition(to: ChargebackStage): void {
    const allowed = TRANSITIONS[this.ctx.stage];
    if (!allowed.includes(to)) {
      throw new Error(`Invalid chargeback transition ${this.ctx.stage} -> ${to}`);
    }
...
export class ChargebackWorkflowService {
  private store = new Map<string, ChargebackWorkflow>();

  start(customerId: string, metadata?: Record<string, unknown>): ChargebackWorkflow {
    const wf = new ChargebackWorkflow(customerId, metadata);
    this.store.set(wf.context.workflowId, wf);
    return wf;
  }

  get(id: string): ChargebackWorkflow | undefined {
    return this.store.get(id);
  }

  listByCustomer(customerId: string): ChargebackContext[] {
    return [...this.store.values()].map((w) => w.context).filter((c) => c.customerId === customerId);
  }

  advanceToSettlement(id: string): ChargebackContext {
    const wf = this.store.get(id);
    if (!wf) throw new Error('Workflow not found');
    const path: ChargebackStage[] = ['validated', 'compliance_check', 'risk_assessment', 'approval_pending', 'approved', 'executing', 'settled'];
