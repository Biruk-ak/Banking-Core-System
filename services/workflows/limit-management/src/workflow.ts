/** LimitManagement workflow — Banking Core System */
export type LimitManagementStage =
  | 'initiated'
  | 'validated'
  | 'compliance_check'
  | 'risk_assessment'
...
export interface LimitManagementContext {
  workflowId: string;
  customerId: string;
  stage: LimitManagementStage;
  amountMinor?: number;
  currency?: string;
  assignee?: string;
  notes: string[];
...
const TRANSITIONS: Record<LimitManagementStage, LimitManagementStage[]> = {
...
export class LimitManagementWorkflow {
  private ctx: LimitManagementContext;
  ...
  get context(): LimitManagementContext { return { ...this.ctx, notes: [...this.ctx.notes], checklist: { ...this.ctx.checklist } }; }
  ...
  check(item: keyof LimitManagementContext['checklist'], value = true): void {
  ...
  transition(to: LimitManagementStage): void {
    const allowed = TRANSITIONS[this.ctx.stage];
    if (!allowed.includes(to)) {
      throw new Error(`Invalid limit-management transition ${this.ctx.stage} -> ${to}`);
    }
...
export class LimitManagementWorkflowService {
  private store = new Map<string, LimitManagementWorkflow>();

  start(customerId: string, metadata?: Record<string, unknown>): LimitManagementWorkflow {
    const wf = new LimitManagementWorkflow(customerId, metadata);
    this.store.set(wf.context.workflowId, wf);
    return wf;
  }

  get(id: string): LimitManagementWorkflow | undefined {
    return this.store.get(id);
  }

  listByCustomer(customerId: string): LimitManagementContext[] {
    return [...this.store.values()].map((w) => w.context).filter((c) => c.customerId === customerId);
  }

  advanceToSettlement(id: string): LimitManagementContext {
    const wf = this.store.get(id);
    if (!wf) throw new Error('Workflow not found');
    const path: LimitManagementStage[] = ['validated', 'compliance_check', 'risk_assessment', 'approval_pending', 'approved', 'executing', 'settled'];
