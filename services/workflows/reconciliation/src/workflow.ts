/** Reconciliation workflow — Banking Core System */
...
const TRANSITIONS: Record<ReconciliationStage, ReconciliationStage[]> = {
...
export class ReconciliationWorkflow {
  private ctx: ReconciliationContext;
  ...
  get context(): ReconciliationContext { return { ...this.ctx, notes: [...this.ctx.notes], checklist: { ...this.ctx.checklist } }; }
  ...
  check(item: keyof ReconciliationContext['checklist'], value = true): void {
  ...
  transition(to: ReconciliationStage): void {
    const allowed = TRANSITIONS[this.ctx.stage];
    if (!allowed.includes(to)) {
      throw new Error(`Invalid reconciliation transition ${this.ctx.stage} -> ${to}`);
    }
...
export class ReconciliationWorkflowService {
  private store = new Map<string, ReconciliationWorkflow>();

  start(customerId: string, metadata?: Record<string, unknown>): ReconciliationWorkflow {
    const wf = new ReconciliationWorkflow(customerId, metadata);
    this.store.set(wf.context.workflowId, wf);
    return wf;
  }

  get(id: string): ReconciliationWorkflow | undefined {
    return this.store.get(id);
  }

  listByCustomer(customerId: string): ReconciliationContext[] {
    return [...this.store.values()].map((w) => w.context).filter((c) => c.customerId === customerId);
  }

  advanceToSettlement(id: string): ReconciliationContext {
    const wf = this.store.get(id);
    if (!wf) throw new Error('Workflow not found');
    const path: ReconciliationStage[] = ['validated', 'compliance_check', 'risk_assessment', 'approval_pending', 'approved', 'executing', 'settled'];
