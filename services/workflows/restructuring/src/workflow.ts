/** Restructuring workflow — Banking Core System */
export type RestructuringStage =
  | 'initiated'
  | 'validated'
  | 'compliance_check'
  | 'risk_assessment'
...
const TRANSITIONS: Record<RestructuringStage, RestructuringStage[]> = {
...
export class RestructuringWorkflow {
  private ctx: RestructuringContext;
  ...
  get context(): RestructuringContext { return { ...this.ctx, notes: [...this.ctx.notes], checklist: { ...this.ctx.checklist } }; }
  ...
  check(item: keyof RestructuringContext['checklist'], value = true): void {
  ...
  transition(to: RestructuringStage): void {
    const allowed = TRANSITIONS[this.ctx.stage];
    if (!allowed.includes(to)) {
      throw new Error(`Invalid restructuring transition ${this.ctx.stage} -> ${to}`);
    }
...
export class RestructuringWorkflowService {
  private store = new Map<string, RestructuringWorkflow>();

  start(customerId: string, metadata?: Record<string, unknown>): RestructuringWorkflow {
    const wf = new RestructuringWorkflow(customerId, metadata);
    this.store.set(wf.context.workflowId, wf);
    return wf;
  }

  get(id: string): RestructuringWorkflow | undefined {
    return this.store.get(id);
  }

  listByCustomer(customerId: string): RestructuringContext[] {
    return [...this.store.values()].map((w) => w.context).filter((c) => c.customerId === customerId);
  }

  advanceToSettlement(id: string): RestructuringContext {
    const wf = this.store.get(id);
    if (!wf) throw new Error('Workflow not found');
    const path: RestructuringStage[] = ['validated', 'compliance_check', 'risk_assessment', 'approval_pending', 'approved', 'executing', 'settled'];
