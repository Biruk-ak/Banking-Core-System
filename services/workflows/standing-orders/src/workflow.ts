/** StandingOrders workflow — Banking Core System */
export type StandingOrdersStage =
  | 'initiated'
  | 'validated'
  | 'compliance_check'
  | 'risk_assessment'
...
export interface StandingOrdersContext {
  workflowId: string;
  customerId: string;
  stage: StandingOrdersStage;
  amountMinor?: number;
  currency?: string;
  assignee?: string;
  notes: string[];
...
const TRANSITIONS: Record<StandingOrdersStage, StandingOrdersStage[]> = {
...
export class StandingOrdersWorkflow {
  private ctx: StandingOrdersContext;
  ...
  get context(): StandingOrdersContext { return { ...this.ctx, notes: [...this.ctx.notes], checklist: { ...this.ctx.checklist } }; }
  ...
  check(item: keyof StandingOrdersContext['checklist'], value = true): void {
  ...
  transition(to: StandingOrdersStage): void {
    const allowed = TRANSITIONS[this.ctx.stage];
    if (!allowed.includes(to)) {
      throw new Error(`Invalid standing-orders transition ${this.ctx.stage} -> ${to}`);
    }
...
export class StandingOrdersWorkflowService {
  private store = new Map<string, StandingOrdersWorkflow>();

  start(customerId: string, metadata?: Record<string, unknown>): StandingOrdersWorkflow {
    const wf = new StandingOrdersWorkflow(customerId, metadata);
    this.store.set(wf.context.workflowId, wf);
    return wf;
  }

  get(id: string): StandingOrdersWorkflow | undefined {
    return this.store.get(id);
  }

  listByCustomer(customerId: string): StandingOrdersContext[] {
    return [...this.store.values()].map((w) => w.context).filter((c) => c.customerId === customerId);
  }

  advanceToSettlement(id: string): StandingOrdersContext {
    const wf = this.store.get(id);
    if (!wf) throw new Error('Workflow not found');
    const path: StandingOrdersStage[] = ['validated', 'compliance_check', 'risk_assessment', 'approval_pending', 'approved', 'executing', 'settled'];
