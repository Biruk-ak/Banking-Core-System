/** Onboarding workflow — Banking Core System */
...
export class OnboardingWorkflow {
  ...
  get context(): OnboardingContext { return { ...this.ctx, notes: [...this.ctx.notes], checklist: { ...this.ctx.checklist } }; }
  ...
  check(item: keyof OnboardingContext['checklist'], value = true): void {
  ...
  transition(to: OnboardingStage): void {
    const allowed = TRANSITIONS[this.ctx.stage];
    if (!allowed.includes(to)) {
      throw new Error(`Invalid onboarding transition ${this.ctx.stage} -> ${to}`);
    }
...
export class OnboardingWorkflowService {
  private store = new Map<string, OnboardingWorkflow>();

  start(customerId: string, metadata?: Record<string, unknown>): OnboardingWorkflow {
    const wf = new OnboardingWorkflow(customerId, metadata);
    this.store.set(wf.context.workflowId, wf);
    return wf;
  }

  get(id: string): OnboardingWorkflow | undefined {
    return this.store.get(id);
  }

  listByCustomer(customerId: string): OnboardingContext[] {
    return [...this.store.values()].map((w) => w.context).filter((c) => c.customerId === customerId);
  }

  advanceToSettlement(id: string): OnboardingContext {
    const wf = this.store.get(id);
    if (!wf) throw new Error('Workflow not found');
    const path: OnboardingStage[] = ['validated', 'compliance_check', 'risk_assessment', 'approval_pending', 'approved', 'executing', 'settled'];
