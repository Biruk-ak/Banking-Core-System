/** ClaimsProcessing workflow — Banking Core System */
export type ClaimsProcessingStage =
  | 'initiated'
  | 'validated'
  | 'compliance_check'
  | 'risk_assessment'
...
export interface ClaimsProcessingContext {
  workflowId: string;
  customerId: string;
  stage: ClaimsProcessingStage;
  amountMinor?: number;
  currency?: string;
  assignee?: string;
  notes: string[];
...
const TRANSITIONS: Record<ClaimsProcessingStage, ClaimsProcessingStage[]> = {
...
export class ClaimsProcessingWorkflow {
  private ctx: ClaimsProcessingContext;
  ...
  get context(): ClaimsProcessingContext { return { ...this.ctx, notes: [...this.ctx.notes], checklist: { ...this.ctx.checklist } }; }
  ...
  check(item: keyof ClaimsProcessingContext['checklist'], value = true): void {
  ...
  transition(to: ClaimsProcessingStage): void {
    const allowed = TRANSITIONS[this.ctx.stage];
    if (!allowed.includes(to)) {
      throw new Error(`Invalid claims-processing transition ${this.ctx.stage} -> ${to}`);
    }
...
export class ClaimsProcessingWorkflowService {
  private store = new Map<string, ClaimsProcessingWorkflow>();

  start(customerId: string, metadata?: Record<string, unknown>): ClaimsProcessingWorkflow {
    const wf = new ClaimsProcessingWorkflow(customerId, metadata);
    this.store.set(wf.context.workflowId, wf);
    return wf;
  }

  get(id: string): ClaimsProcessingWorkflow | undefined {
    return this.store.get(id);
  }

  listByCustomer(customerId: string): ClaimsProcessingContext[] {
    return [...this.store.values()].map((w) => w.context).filter((c) => c.customerId === customerId);
  }

  advanceToSettlement(id: string): ClaimsProcessingContext {
    const wf = this.store.get(id);
    if (!wf) throw new Error('Workflow not found');
    const path: ClaimsProcessingStage[] = ['validated', 'compliance_check', 'risk_assessment', 'approval_pending', 'approved', 'executing', 'settled'];
