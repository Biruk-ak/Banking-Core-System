/** FxSettlement workflow — Banking Core System */
export type FxSettlementStage =
  | 'initiated'
  | 'validated'
  | 'compliance_check'
  | 'risk_assessment'
...
export interface FxSettlementContext {
  workflowId: string;
  customerId: string;
  stage: FxSettlementStage;
  amountMinor?: number;
  currency?: string;
  assignee?: string;
  notes: string[];
...
const TRANSITIONS: Record<FxSettlementStage, FxSettlementStage[]> = {
...
export class FxSettlementWorkflow {
  private ctx: FxSettlementContext;
  ...
  get context(): FxSettlementContext { return { ...this.ctx, notes: [...this.ctx.notes], checklist: { ...this.ctx.checklist } }; }
  ...
  check(item: keyof FxSettlementContext['checklist'], value = true): void {
  ...
  transition(to: FxSettlementStage): void {
    const allowed = TRANSITIONS[this.ctx.stage];
    if (!allowed.includes(to)) {
      throw new Error(`Invalid fx-settlement transition ${this.ctx.stage} -> ${to}`);
    }
...
export class FxSettlementWorkflowService {
  private store = new Map<string, FxSettlementWorkflow>();

  start(customerId: string, metadata?: Record<string, unknown>): FxSettlementWorkflow {
    const wf = new FxSettlementWorkflow(customerId, metadata);
    this.store.set(wf.context.workflowId, wf);
    return wf;
  }

  get(id: string): FxSettlementWorkflow | undefined {
    return this.store.get(id);
  }

  listByCustomer(customerId: string): FxSettlementContext[] {
    return [...this.store.values()].map((w) => w.context).filter((c) => c.customerId === customerId);
  }

  advanceToSettlement(id: string): FxSettlementContext {
    const wf = this.store.get(id);
    if (!wf) throw new Error('Workflow not found');
    const path: FxSettlementStage[] = ['validated', 'compliance_check', 'risk_assessment', 'approval_pending', 'approved', 'executing', 'settled'];
