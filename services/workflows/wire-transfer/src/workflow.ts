/** WireTransfer workflow — Banking Core System */
export type WireTransferStage =
  | 'initiated'
  | 'validated'
  | 'compliance_check'
  | 'risk_assessment'
...
export interface WireTransferContext {
  workflowId: string;
  customerId: string;
  stage: WireTransferStage;
  amountMinor?: number;
  currency?: string;
  assignee?: string;
  notes: string[];
...
const TRANSITIONS: Record<WireTransferStage, WireTransferStage[]> = {
...
export class WireTransferWorkflow {
  private ctx: WireTransferContext;
  ...
  get context(): WireTransferContext { return { ...this.ctx, notes: [...this.ctx.notes], checklist: { ...this.ctx.checklist } }; }
  ...
  check(item: keyof WireTransferContext['checklist'], value = true): void {
  ...
  transition(to: WireTransferStage): void {
    const allowed = TRANSITIONS[this.ctx.stage];
    if (!allowed.includes(to)) {
      throw new Error(`Invalid wire-transfer transition ${this.ctx.stage} -> ${to}`);
    }
...
export class WireTransferWorkflowService {
  private store = new Map<string, WireTransferWorkflow>();

  start(customerId: string, metadata?: Record<string, unknown>): WireTransferWorkflow {
    const wf = new WireTransferWorkflow(customerId, metadata);
    this.store.set(wf.context.workflowId, wf);
    return wf;
  }

  get(id: string): WireTransferWorkflow | undefined {
    return this.store.get(id);
  }

  listByCustomer(customerId: string): WireTransferContext[] {
    return [...this.store.values()].map((w) => w.context).filter((c) => c.customerId === customerId);
  }

  advanceToSettlement(id: string): WireTransferContext {
    const wf = this.store.get(id);
    if (!wf) throw new Error('Workflow not found');
    const path: WireTransferStage[] = ['validated', 'compliance_check', 'risk_assessment', 'approval_pending', 'approved', 'executing', 'settled'];
