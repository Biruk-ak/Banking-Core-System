/** Collections workflow — Banking Core System */
...
const TRANSITIONS: Record<CollectionsStage, CollectionsStage[]> = {
...
export class CollectionsWorkflow {
  private ctx: CollectionsContext;
  ...
  get context(): CollectionsContext { return { ...this.ctx, notes: [...this.ctx.notes], checklist: { ...this.ctx.checklist } }; }
  ...
  check(item: keyof CollectionsContext['checklist'], value = true): void {
  ...
  transition(to: CollectionsStage): void {
    const allowed = TRANSITIONS[this.ctx.stage];
    if (!allowed.includes(to)) {
      throw new Error(`Invalid collections transition ${this.ctx.stage} -> ${to}`);
    }
...
export class CollectionsWorkflowService {
  private store = new Map<string, CollectionsWorkflow>();

  start(customerId: string, metadata?: Record<string, unknown>): CollectionsWorkflow {
    const wf = new CollectionsWorkflow(customerId, metadata);
    this.store.set(wf.context.workflowId, wf);
    return wf;
  }

  get(id: string): CollectionsWorkflow | undefined {
    return this.store.get(id);
  }

  listByCustomer(customerId: string): CollectionsContext[] {
    return [...this.store.values()].map((w) => w.context).filter((c) => c.customerId === customerId);
  }

  advanceToSettlement(id: string): CollectionsContext {
    const wf = this.store.get(id);
    if (!wf) throw new Error('Workflow not found');
    const path: CollectionsStage[] = ['validated', 'compliance_check', 'risk_assessment', 'approval_pending', 'approved', 'executing', 'settled'];
