import type { SepaTransferContext } from './workflow';
...
export function validateSepaTransfer(ctx: SepaTransferContext): ValidationIssue[] {
...
export function assertValidSepaTransfer(ctx: SepaTransferContext): void {
  const issues = validateSepaTransfer(ctx);
