/** SepaTransfer workflow — Banking Core System */
export type SepaTransferStage =
  | 'initiated'
  | 'validated'
  | 'compliance_check'
  | 'risk_assessment'
...
export interface SepaTransferContext {
  workflowId: string;
  customerId: string;
  stage: SepaTransferStage;
  amountMinor?: number;
  currency?: string;
  assignee?: string;
  notes: string[];
...
const TRANSITIONS: Record<SepaTransferStage, SepaTransferStage[]> = {
...
export class SepaTransferWorkflow {
  private ctx: SepaTransferContext;
  ...
  get context(): SepaTransferContext { return { ...this.ctx, notes: [...this.ctx.notes], checklist: { ...this.ctx.checklist } }; }
  ...
  check(item: keyof SepaTransferContext['checklist'], value = true): void {
  ...
  transition(to: SepaTransferStage): void {
    const allowed = TRANSITIONS[this.ctx.stage];
    if (!allowed.includes(to)) {
      throw new Error(`Invalid sepa-transfer transition ${this.ctx.stage} -> ${to}`);
    }
...
export class SepaTransferWorkflowService {
  private store = new Map<string, SepaTransferWorkflow>();

  start(customerId: string, metadata?: Record<string, unknown>): SepaTransferWorkflow {
    const wf = new SepaTransferWorkflow(customerId, metadata);
    this.store.set(wf.context.workflowId, wf);
    return wf;
  }

  get(id: string): SepaTransferWorkflow | undefined {
    return this.store.get(id);
  }

  listByCustomer(customerId: string): SepaTransferContext[] {
    return [...this.store.values()].map((w) => w.context).filter((c) => c.customerId === customerId);
  }

  advanceToSettlement(id: string): SepaTransferContext {
    const wf = this.store.get(id);
    if (!wf) throw new Error('Workflow not found');
    const path: SepaTransferStage[] = ['validated', 'compliance_check', 'risk_assessment', 'approval_pending', 'approved', 'executing', 'settled'];
