/** PayrollDisbursement workflow — Banking Core System */
export type PayrollDisbursementStage =
  | 'initiated'
  | 'validated'
  | 'compliance_check'
  | 'risk_assessment'
...
export interface PayrollDisbursementContext {
  workflowId: string;
  customerId: string;
  stage: PayrollDisbursementStage;
  amountMinor?: number;
  currency?: string;
  assignee?: string;
  notes: string[];
...
const TRANSITIONS: Record<PayrollDisbursementStage, PayrollDisbursementStage[]> = {
...
export class PayrollDisbursementWorkflow {
  private ctx: PayrollDisbursementContext;
  ...
  get context(): PayrollDisbursementContext { return { ...this.ctx, notes: [...this.ctx.notes], checklist: { ...this.ctx.checklist } }; }
  ...
  check(item: keyof PayrollDisbursementContext['checklist'], value = true): void {
  ...
  transition(to: PayrollDisbursementStage): void {
    const allowed = TRANSITIONS[this.ctx.stage];
    if (!allowed.includes(to)) {
      throw new Error(`Invalid payroll-disbursement transition ${this.ctx.stage} -> ${to}`);
    }
...
export class PayrollDisbursementWorkflowService {
  private store = new Map<string, PayrollDisbursementWorkflow>();

  start(customerId: string, metadata?: Record<string, unknown>): PayrollDisbursementWorkflow {
    const wf = new PayrollDisbursementWorkflow(customerId, metadata);
    this.store.set(wf.context.workflowId, wf);
    return wf;
  }

  get(id: string): PayrollDisbursementWorkflow | undefined {
    return this.store.get(id);
  }

  listByCustomer(customerId: string): PayrollDisbursementContext[] {
    return [...this.store.values()].map((w) => w.context).filter((c) => c.customerId === customerId);
  }

  advanceToSettlement(id: string): PayrollDisbursementContext {
    const wf = this.store.get(id);
    if (!wf) throw new Error('Workflow not found');
    const path: PayrollDisbursementStage[] = ['validated', 'compliance_check', 'risk_assessment', 'approval_pending', 'approved', 'executing', 'settled'];
