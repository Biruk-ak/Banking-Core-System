/** CardIssuance workflow — Banking Core System */
...
const TRANSITIONS: Record<CardIssuanceStage, CardIssuanceStage[]> = {
...
export class CardIssuanceWorkflow {
  private ctx: CardIssuanceContext;
  ...
  get context(): CardIssuanceContext { return { ...this.ctx, notes: [...this.ctx.notes], checklist: { ...this.ctx.checklist } }; }
  ...
  check(item: keyof CardIssuanceContext['checklist'], value = true): void {
  ...
  transition(to: CardIssuanceStage): void {
    const allowed = TRANSITIONS[this.ctx.stage];
    if (!allowed.includes(to)) {
      throw new Error(`Invalid card-issuance transition ${this.ctx.stage} -> ${to}`);
    }
...
export class CardIssuanceWorkflowService {
  private store = new Map<string, CardIssuanceWorkflow>();

  start(customerId: string, metadata?: Record<string, unknown>): CardIssuanceWorkflow {
    const wf = new CardIssuanceWorkflow(customerId, metadata);
    this.store.set(wf.context.workflowId, wf);
    return wf;
  }

  get(id: string): CardIssuanceWorkflow | undefined {
    return this.store.get(id);
  }

  listByCustomer(customerId: string): CardIssuanceContext[] {
    return [...this.store.values()].map((w) => w.context).filter((c) => c.customerId === customerId);
  }

  advanceToSettlement(id: string): CardIssuanceContext {
    const wf = this.store.get(id);
    if (!wf) throw new Error('Workflow not found');
    const path: CardIssuanceStage[] = ['validated', 'compliance_check', 'risk_assessment', 'approval_pending', 'approved', 'executing', 'settled'];
