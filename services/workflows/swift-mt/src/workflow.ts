/** SwiftMt workflow — Banking Core System */
export type SwiftMtStage =
  | 'initiated'
  | 'validated'
  | 'compliance_check'
  | 'risk_assessment'
...
export interface SwiftMtContext {
  workflowId: string;
  customerId: string;
  stage: SwiftMtStage;
  amountMinor?: number;
...
const TRANSITIONS: Record<SwiftMtStage, SwiftMtStage[]> = {
...
export class SwiftMtWorkflow {
  private ctx: SwiftMtContext;
  ...
  get context(): SwiftMtContext { return { ...this.ctx, notes: [...this.ctx.notes], checklist: { ...this.ctx.checklist } }; }
  ...
  check(item: keyof SwiftMtContext['checklist'], value = true): void {
  ...
  transition(to: SwiftMtStage): void {
    const allowed = TRANSITIONS[this.ctx.stage];
    if (!allowed.includes(to)) {
      throw new Error(`Invalid swift-mt transition ${this.ctx.stage} -> ${to}`);
    }
...
export class SwiftMtWorkflowService {
  private store = new Map<string, SwiftMtWorkflow>();

  start(customerId: string, metadata?: Record<string, unknown>): SwiftMtWorkflow {
    const wf = new SwiftMtWorkflow(customerId, metadata);
    this.store.set(wf.context.workflowId, wf);
    return wf;
  }

  get(id: string): SwiftMtWorkflow | undefined {
    return this.store.get(id);
  }

  listByCustomer(customerId: string): SwiftMtContext[] {
    return [...this.store.values()].map((w) => w.context).filter((c) => c.customerId === customerId);
  }

  advanceToSettlement(id: string): SwiftMtContext {
    const wf = this.store.get(id);
    if (!wf) throw new Error('Workflow not found');
    const path: SwiftMtStage[] = ['validated', 'compliance_check', 'risk_assessment', 'approval_pending', 'approved', 'executing', 'settled'];
