/** LoanOrigination workflow — Banking Core System */
export type LoanOriginationStage =
  | 'initiated'
  | 'validated'
  | 'compliance_check'
  | 'risk_assessment'
...
export interface LoanOriginationContext {
  workflowId: string;
  customerId: string;
  stage: LoanOriginationStage;
  amountMinor?: number;
  currency?: string;
  assignee?: string;
  notes: string[];
...
const TRANSITIONS: Record<LoanOriginationStage, LoanOriginationStage[]> = {
...
export class LoanOriginationWorkflow {
  private ctx: LoanOriginationContext;
  ...
  get context(): LoanOriginationContext { return { ...this.ctx, notes: [...this.ctx.notes], checklist: { ...this.ctx.checklist } }; }
  ...
  check(item: keyof LoanOriginationContext['checklist'], value = true): void {
  ...
  transition(to: LoanOriginationStage): void {
    const allowed = TRANSITIONS[this.ctx.stage];
    if (!allowed.includes(to)) {
      throw new Error(`Invalid loan-origination transition ${this.ctx.stage} -> ${to}`);
    }
...
export class LoanOriginationWorkflowService {
  private store = new Map<string, LoanOriginationWorkflow>();

  start(customerId: string, metadata?: Record<string, unknown>): LoanOriginationWorkflow {
    const wf = new LoanOriginationWorkflow(customerId, metadata);
    this.store.set(wf.context.workflowId, wf);
    return wf;
  }

  get(id: string): LoanOriginationWorkflow | undefined {
    return this.store.get(id);
  }

  listByCustomer(customerId: string): LoanOriginationContext[] {
    return [...this.store.values()].map((w) => w.context).filter((c) => c.customerId === customerId);
  }

  advanceToSettlement(id: string): LoanOriginationContext {
    const wf = this.store.get(id);
    if (!wf) throw new Error('Workflow not found');
    const path: LoanOriginationStage[] = ['validated', 'compliance_check', 'risk_assessment', 'approval_pending', 'approved', 'executing', 'settled'];
