...
import type { InvestmentsStatus, InvestmentsRiskTier, InvestmentsFeeSchedule, InvestmentsLimits, InvestmentsSnapshot, InvestmentsAuditTrail } from './types';
export class InvestmentsAggregate extends AggregateRoot {
  private status: InvestmentsStatus = 'draft';
  private riskTier: InvestmentsRiskTier = 'medium';
  private balance: Money;
  private version = 0;
  private audit: InvestmentsAuditTrail[] = [];
  private tags: string[] = [];
  ...
  private constructor(
    ...
    private fees: InvestmentsFeeSchedule,
    private limits: InvestmentsLimits,
  ) {
    super(id);
  ...
  static open(
    ...
    const agg = new InvestmentsAggregate(id, customerId, currency, fees, limits);
    agg.status = 'pending';
    agg.openedAt = new Date();
    agg.record('OPENED', actorId, { currency });
    agg.addDomainEvent(createEvent(id, 'investments.opened', { customerId, currency }));
    return agg;
  }
  activate(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'investments.activated', { customerId: this.customerId }));
  }
  suspend(actorId: string, reason: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'investments.suspended', { reason }));
  }
  close(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'investments.closed', {}));
  }
  credit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'investments.credited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  debit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'investments.debited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  ...
  updateLimits(limits: Partial<InvestmentsLimits>, actorId: string): void {
  ...
  toSnapshot(): InvestmentsSnapshot {
    return {
      id: this.id,
  ...
  getAuditTrail(): InvestmentsAuditTrail[] { return [...this.audit]; }
