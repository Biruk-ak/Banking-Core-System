import type { InvestmentsSnapshot, InvestmentsRiskTier } from './types';
export interface PolicyDecision {
  allowed: boolean;
  reasons: string[];
  requiredApprovals: number;
  riskImpact: InvestmentsRiskTier;
}
export class InvestmentsPolicyEngine {
  ...
  evaluateLargeDebit(snap: InvestmentsSnapshot, amountMinor: number): PolicyDecision {
    const reasons: string[] = [];
  ...
  evaluateClosure(snap: InvestmentsSnapshot): PolicyDecision {
    const reasons: string[] = [];
  ...
  scoreInterestEligibility(snap: InvestmentsSnapshot, daysActive: number): number {
    let score = 50;
  ...
  computeProvisioning(snap: InvestmentsSnapshot): number {
    const rates: Record<InvestmentsRiskTier, number> = { low: 0.01, medium: 0.03, high: 0.1, critical: 0.25 };
