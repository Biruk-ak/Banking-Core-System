import type { InvestmentsSnapshot, InvestmentsAuditTrail } from '../domain/types';
import type { InvestmentsListItem, Page } from '../application/queries';
export interface InvestmentsReadModel {
  byId(id: string): Promise<InvestmentsSnapshot | null>;
  byCustomer(customerId: string, status?: string): Promise<InvestmentsListItem[]>;
  search(q: string, riskTier: string | undefined, page: number, pageSize: number): Promise<Page<InvestmentsListItem>>;
  audit(productId: string, limit: number): Promise<InvestmentsAuditTrail[]>;
...
export class ElasticInvestmentsReadModel implements InvestmentsReadModel {
  constructor(
    private readonly es: {
      search: (body: unknown) => Promise<{ hits: { hits: { _source: InvestmentsSnapshot }[]; total: { value: number } } }>;
      get: (id: string) => Promise<{ _source: InvestmentsSnapshot } | null>;
    },
    private readonly index = 'investments-read',
  ) {}
  async byId(id: string): Promise<InvestmentsSnapshot | null> {
    const doc = await this.es.get(id);
    return doc?._source ?? null;
  }
  async byCustomer(customerId: string, status?: string): Promise<InvestmentsListItem[]> {
  ...
  async search(q: string, riskTier: string | undefined, page: number, pageSize: number): Promise<Page<InvestmentsListItem>> {
  ...
  async audit(_productId: string, _limit: number): Promise<InvestmentsAuditTrail[]> {
    return [];
  }
  ...
  private toListItem(s: InvestmentsSnapshot): InvestmentsListItem {
    return {
      id: s.id,
...
export class InMemoryInvestmentsReadModel implements InvestmentsReadModel {
  private docs = new Map<string, InvestmentsSnapshot>();
  private audits = new Map<string, InvestmentsAuditTrail[]>();
  upsert(snap: InvestmentsSnapshot, audit: InvestmentsAuditTrail[] = []): void {
