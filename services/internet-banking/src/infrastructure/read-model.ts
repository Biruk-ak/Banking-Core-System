import type { InternetBankingSnapshot, InternetBankingAuditTrail } from '../domain/types';
import type { InternetBankingListItem, Page } from '../application/queries';
export interface InternetBankingReadModel {
  byId(id: string): Promise<InternetBankingSnapshot | null>;
  byCustomer(customerId: string, status?: string): Promise<InternetBankingListItem[]>;
  search(q: string, riskTier: string | undefined, page: number, pageSize: number): Promise<Page<InternetBankingListItem>>;
  audit(productId: string, limit: number): Promise<InternetBankingAuditTrail[]>;
...
export class ElasticInternetBankingReadModel implements InternetBankingReadModel {
  constructor(
    private readonly es: {
      search: (body: unknown) => Promise<{ hits: { hits: { _source: InternetBankingSnapshot }[]; total: { value: number } } }>;
      get: (id: string) => Promise<{ _source: InternetBankingSnapshot } | null>;
    },
    private readonly index = 'internet-banking-read',
  ) {}
  async byId(id: string): Promise<InternetBankingSnapshot | null> {
    const doc = await this.es.get(id);
    return doc?._source ?? null;
  }
  async byCustomer(customerId: string, status?: string): Promise<InternetBankingListItem[]> {
  ...
  async search(q: string, riskTier: string | undefined, page: number, pageSize: number): Promise<Page<InternetBankingListItem>> {
  ...
  async audit(_productId: string, _limit: number): Promise<InternetBankingAuditTrail[]> {
    return [];
  }
  ...
  private toListItem(s: InternetBankingSnapshot): InternetBankingListItem {
    return {
      id: s.id,
...
export class InMemoryInternetBankingReadModel implements InternetBankingReadModel {
  private docs = new Map<string, InternetBankingSnapshot>();
  private audits = new Map<string, InternetBankingAuditTrail[]>();
  upsert(snap: InternetBankingSnapshot, audit: InternetBankingAuditTrail[] = []): void {
