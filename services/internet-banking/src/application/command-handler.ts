...
import type { InternetBankingCommand, CommandResult } from './commands';
import type { InternetBankingFeeSchedule, InternetBankingLimits } from '../domain/types';
import type { EventBus } from '../../../shared/infra/event-bus';
import type { InternetBankingRepository } from '../infrastructure/repository';
const DEFAULT_FEES: InternetBankingFeeSchedule = {
...
const DEFAULT_LIMITS: InternetBankingLimits = {
...
export class InternetBankingCommandHandler {
  constructor(
    private readonly repo: InternetBankingRepository,
    private readonly bus: EventBus,
  ) {}
  async handle(cmd: InternetBankingCommand): Promise<CommandResult> {
    try {
      switch (cmd.type) {
        case 'OpenInternetBanking': {
  ...
  private async load(id: string): Promise<InternetBankingAggregate> {
    const found = await this.repo.findById(id);
    if (!found) throw new Error('InternetBanking not found: ' + id);
    return found;
  }
  private async persist(agg: InternetBankingAggregate): Promise<void> {
