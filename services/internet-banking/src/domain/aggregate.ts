...
import type { InternetBankingStatus, InternetBankingRiskTier, InternetBankingFeeSchedule, InternetBankingLimits, InternetBankingSnapshot, InternetBankingAuditTrail } from './types';
export class InternetBankingAggregate extends AggregateRoot {
  private status: InternetBankingStatus = 'draft';
  private riskTier: InternetBankingRiskTier = 'medium';
  private balance: Money;
  private version = 0;
  private audit: InternetBankingAuditTrail[] = [];
  private tags: string[] = [];
  ...
  private constructor(
    ...
    private fees: InternetBankingFeeSchedule,
    private limits: InternetBankingLimits,
  ) {
    super(id);
  ...
  static open(
    ...
    actorId: string,
  ): InternetBankingAggregate {
    const agg = new InternetBankingAggregate(id, customerId, currency, fees, limits);
    agg.status = 'pending';
    agg.openedAt = new Date();
    agg.record('OPENED', actorId, { currency });
    agg.addDomainEvent(createEvent(id, 'internet-banking.opened', { customerId, currency }));
    return agg;
  }
  activate(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'internet-banking.activated', { customerId: this.customerId }));
  }
  suspend(actorId: string, reason: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'internet-banking.suspended', { reason }));
  }
  close(actorId: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'internet-banking.closed', {}));
  }
  credit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'internet-banking.credited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  debit(amount: Money, actorId: string, reference: string): void {
    ...
    this.addDomainEvent(createEvent(this.id, 'internet-banking.debited', { reference, amountMinor: Number(amount.amountMinor) }));
  }
  ...
  updateLimits(limits: Partial<InternetBankingLimits>, actorId: string): void {
  ...
  toSnapshot(): InternetBankingSnapshot {
    return {
      id: this.id,
  ...
  getAuditTrail(): InternetBankingAuditTrail[] { return [...this.audit]; }
