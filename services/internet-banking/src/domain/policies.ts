import type { InternetBankingSnapshot, InternetBankingRiskTier } from './types';
export interface PolicyDecision {
  allowed: boolean;
  reasons: string[];
  requiredApprovals: number;
  riskImpact: InternetBankingRiskTier;
}
export class InternetBankingPolicyEngine {
  ...
  evaluateLargeDebit(snap: InternetBankingSnapshot, amountMinor: number): PolicyDecision {
    const reasons: string[] = [];
  ...
  evaluateClosure(snap: InternetBankingSnapshot): PolicyDecision {
    const reasons: string[] = [];
  ...
  scoreInterestEligibility(snap: InternetBankingSnapshot, daysActive: number): number {
    let score = 50;
  ...
  computeProvisioning(snap: InternetBankingSnapshot): number {
    const rates: Record<InternetBankingRiskTier, number> = { low: 0.01, medium: 0.03, high: 0.1, critical: 0.25 };
